...
export function StudentStart({ onStart, week = 1, onWeekChange }) {
  ...
  return <div className="welcome-page">
    ...
    <main className="welcome-main">
      ...
      {profiles.length > 0 && <section className="saved-students" aria-labelledby="saved-title"><div><h2 id="saved-title">Coming back?</h2><p>Choose your own saved work on this browser.</p></div><div className="saved-profile-grid">{profiles.map(profile => { const stats = week === 3 ? {passed:week3Progress(profile.week3Work || {})} : week === 2 ? week2Stats(profile.week2Work || {}) : progressSummary(profile.work || {}); return <button className="saved-profile" key={profile.id} onClick={() => onStart(profile)} aria-label={`Continue ${profile.name}, ${profile.className}`}><span className="student-avatar">{profile.name.slice(0, 1).toUpperCase()}</span><span><b>{profile.name}</b><small>{profile.className} · {stats.passed}/{week === 3 ? 6 : week === 2 ? coreChecks : 6} practice checks</small></span><ChevronRight size={20}/></button>; })}</div></section>}
