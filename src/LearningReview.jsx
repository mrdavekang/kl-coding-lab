...
import { learningGroups, learningTopics as defaultTopics, startingPoints, learningStages, cleanLearningReview, startingAdvice, suggestedAction } from './learningReview.js';

export function LearningReview({ after = false, work, setWork, onOpen, onContinue, onBack, saved, topics = defaultTopics, checkpointLabel = "Learning pit stop" }) {
  ...
  const selected = topics.find(t => t.id === (after ? state.priority : state.focus));
  ...
  function answer(id, value) { setWork(old => { const review = cleanLearningReview(old.learningReview, topics); const key = after ? 'after' : 'before'; return { ...old, learningReview: { ...review, [key]: { ...review[key], [id]: value } } }; }); }
  return <section className="learning-review" aria-labelledby="learning-review-title">
    <div className="review-heading"><span className="card-type">{after ? 'Pause · look at your work · choose a next step' : 'After your Do Now · choose a learning focus'}</span>
      <h2 id="learning-review-title">{after ? checkpointLabel + ': where am I now?' : 'Types of learning: what do I need to get better at?'}</h2>
    ...
    {after && <div className="review-starting-focus"><Compass size={20}/><p><b>Your starting focus</b><br/>{topics.find(t => t.id === state.focus)?.title || 'You have not chosen one yet. You can still reflect on what you tried.'}</p></div>}
    ...
    <div className="review-layout"><div>
      <div className="review-groups" role="group" aria-label="Kinds of learning">{learningGroups.map((item, n) => <button key={item.name} aria-pressed={groupIndex === n} onClick={() => setGroupIndex(n)}><b>{item.name}</b><span>{item.meaning}</span><small>{topics.filter(t => t.group === item.name && responses[t.id]).length} / 2 reflected on</small></button>)}</div>
      <div className="review-questions" aria-label={group.name + ' reflection'}><h3>{after && group.name === 'Knowledge' ? 'What do I know now?' : group.question}</h3>
        {topics.filter(t => t.group === group.name).map(topic => { const next = suggestedAction(topic, responses[topic.id]); const choice = (after ? learningStages : startingPoints).find(c => c.id === responses[topic.id]); return <div className="review-question" key={topic.id}>
      ...
      <p className="review-count">{Object.keys(responses).length} of {topics.length} statements reflected on</p>
      <p>{after ? 'Choose one topic. Use its suggestion, explain your choice or ask for help.' : startingAdvice(state, topics)}</p>
      <label htmlFor="review-focus">{after ? 'Which part should I focus on next?' : 'What do I want to get better at?'}</label>
      <select id="review-focus" value={after ? state.priority : state.focus} onChange={e => update({ [after ? 'priority' : 'focus']: e.target.value })}><option value="">Choose one topic</option>{learningGroups.map(item => <optgroup key={item.name} label={item.name}>{topics.filter(t => t.group === item.name).map(topic => <option key={topic.id} value={topic.id}>{topic.title}</option>)}</optgroup>)}</select>
