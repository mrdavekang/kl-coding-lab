...
Week 3 preserves the same ten timed stages. Each practice places a short explanation and analogous worked example before an independent exercise, with sample output, an editable example sandbox, practice hints and an explanation prompt. The starter route shows a complete worked example, one idea and three actions in reading order. Gap/Fence pictures show column counts without clicks. There are no student lesson accordions: explanations, code controls, feedback, reflection choices and the lesson map are visible. Reflections use radio choices instead of dropdowns, and Next buttons name the destination. The optional tasks retain fuller concept explanations and worked tables. The optional-task example editor opens beside its example and remains separate from the student solution. Vocabulary such as minimum, index, condition and dictionary is taught where first needed. Larger reading text, a restrained colour palette and a lesson map support KS2/KS3 learners. Main Task 1 offers **three answer-file practices**; Main Task 2 now offers **three supported practices**: run a working counter, change a comparison symbol, and change an update number. The original five Python tasks are optional More practice, with their IDs and saved work retained. The main route has six starter checks across the two main tasks. Supported checks are not evidence of independent programming. Students pause for both pit stops even with unfinished practices.
